...
int64 nChainStartTime = 1511354323;
...
const string strMessageMagic = "mmcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight >= 1 && nHeight <= 9000)
	{
        nSubsidy = 1.9 * COIN;
	}
    else if(nHeight > 9000 && nHeight <= 19000)
        {
            nSubsidy = 5000 * COIN;
        }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in mmcoin
    bool fStrictPayToScriptHash = true; // Always active in mmcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "mmcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "CBigNum vector mmcoin empty mapBlockIndex mmcoin mmcoin";
        CTransaction txNew;
       ...
       // printf("block.nNonce = %u \n", block.nNonce);
		block.print();
        assert(block.hashMerkleRoot == uint256("0xa3681ead6c2b934dc8552f3c2aa749b6edb1e0540eb77cd183a9dc93ec8372ed"));
